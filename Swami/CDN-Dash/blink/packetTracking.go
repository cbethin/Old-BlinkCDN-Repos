...
func extractPacketTrackLatency(p Packet) { // SET BACK TO MAP
	...
	if totalTripTime < 0 {
		fmt.Println(p.Number, "Error in total trip time calculation:", p.Time1, p.Time3)
		return
	}
	...
	if p.Number%5 == 0 {
		go firebase_T1_T3(latencyData)
	}
	fmt.Println(p.Number, "-", totalTripTime.Seconds())

	// Hop 1
	t1_t2 := t2.Sub(t1)
	if t1_t2 < 0 {
		fmt.Println(p.Number, "Error in trip 1 time calculation:", p.Time1, p.Time2)
	}
	...
	if p.Number%5 == 0 {
		go firebase_T1_T2(latencyData)
	}

	// Hop 2
	t2_t3 := t3.Sub(t2)
	if t2_t3 < 0 {
		fmt.Println(p.Number, "Error in trip 2 time calculation:", p.Time2, p.Time3)
	}
