...
func GetData() {
	...
	for {
		...
		fmt.Println("Receieved:", packetType)
...
var (
	...
	oracleRoutingTable map[string][3]*Link
)
...
func StartOracle(oracleAddrString string) {
	...
	nodesTable = [4]string{"155.246.211.96:4000", "155.246.198.81:4000", "155.246.214.64:4000", "155.246.207.73:4000"}
	...
	newRoutingTable := make(map[string][3]*Link)
	newRoutingTable[nodesTable[0]] = [3]*Link{&newLink2, &newLink3, &newLink4}
	newRoutingTable[nodesTable[1]] = [3]*Link{&newLink1, &newLink3, &newLink4}
	newRoutingTable[nodesTable[2]] = [3]*Link{&newLink1, &newLink2, &newLink4}
	newRoutingTable[nodesTable[3]] = [3]*Link{&newLink1, &newLink2, &newLink3}
	...
	for {
		...
		case BounceUpdate:
			packetData := ExtractPacketData(buffer)
...
func SetupServer(serverAddr *net.UDPAddr, conn *net.UDPConn, buf []byte) {
	...
	decisionTable := make(map[string][3]string)
	decisionTable[SID] = [3]string{nodesTable[0], nodesTable[2], destAddr.String()}
	DecisionTable = decisionTable
...
func SelectPath(conn *net.UDPConn) {
	...
	bluePath := [3]string{nodesTable[0], nodesTable[2], nodesTable[3]}
	redPath := [3]string{nodesTable[0], nodesTable[1], nodesTable[3]}
