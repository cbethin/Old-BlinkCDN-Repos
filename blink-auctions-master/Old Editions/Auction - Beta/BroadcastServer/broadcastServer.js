...
io.sockets.on('connection', function(socket) {
      ...
      socket.on('client disconnect', function(userID, roomName) {
...
function onDisconnect(userID, roomName) {
    ...
    if(streamRooms[roomName]) {
      ...
      // If there's only 1 client left get rid of the clients
      if (clientsInRoom.length == 1) {
        streamRooms[roomName].clients = null;
        return;
      }
...
function onJoin(userID, socket, roomName, isBroadcaster) {
  ...
  if (isBroadcaster == true) {
    ...
    if (!streamRooms[roomName] || streamRooms[roomName].clients == null) {
