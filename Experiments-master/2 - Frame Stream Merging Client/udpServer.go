	...
	for {
		...
		if string(buffer[0:4]) == "play" {
		  ...
		  fmt.Println("Received:", string(buffer[:10]), "\nSize:", len(buffer), "bytes | From:", addr.String())
			...
			var sequenceNumber int = frameNumber
		  var I int = 0
		  var P int = 0
		  var B int = 0

		  for sequenceNumber <= NUM_PACKETS {
		    message := strconv.Itoa(sequenceNumber) + " I " + strconv.Itoa(I)
				fmt.Println(message)
				copy(buffer[:], message)
		    ...
		    for B <= 2 {
		      message = strconv.Itoa(sequenceNumber) + " B " + strconv.Itoa(B)
					fmt.Println(message)
					copy(buffer[:], message)
					_, err = conn.WriteToUDP(buffer, addr)
					checkError(err)
		    ...
		    for P <= 0 {
		      message = strconv.Itoa(sequenceNumber) + " P " + strconv.Itoa(P)
					fmt.Println(message)
					copy(buffer[:], message)
					_, err = conn.WriteToUDP(buffer, addr)
					checkError(err)
