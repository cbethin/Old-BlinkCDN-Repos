...
func SetProbeFilename(filenameInput string) {
  probeFilename = filenameInput
}

func SetDataFilename(filenameInput string) {
...
func SendResponse2(link1 *Link) {
	...
	// Print Probe's Rount Trip Time to a csv file named "file.csv"
	file, err := os.OpenFile(probeFilename, os.O_APPEND|os.O_WRONLY, 0600)
	CheckError(err)
	printString := link1.Addr + " " + time.Now().String() + " " + timeElapsed.String() + "\n"
...
func Bounce(buf []byte) {
	...
	file, err := os.OpenFile(dataFilename, os.O_APPEND|os.O_WRONLY, 0600)
	CheckError(err)
...
func StartProber() {
	...
	//a
	file, err := os.Create(probeFilename)
	CheckError(err)
	defer file.Close()
...
func GetData() {
	...
	// CREATE FILE
	dataFilename = strconv.Itoa(localAddr.Port) + ".csv"
...
func StartOracle(oracleAddrString string) {
	...
	nodesTable = []string{"13.58.50.219:6000", "34.207.145.18:6000", "54.183.225.173:6000", "34.213.5.244:6000"}
	...
	//Setup the data file
	dataFilename = strconv.Itoa(oracleAddr.Port) + ".txt"
	file, err := os.Create(dataFilename)
	CheckError(err)
	defer file.Close()
