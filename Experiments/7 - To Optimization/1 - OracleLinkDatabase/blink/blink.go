...
func SetDatabaseFilename(filename string) {
...
func LinkToByteArray(link Link) []byte {
	...
	copy(byteArray[48:80], link.SrcAddr+"!")
	return byteArray
}
...
func ByteArrayToLink(byteArray []byte) Link {
	...
	addressString := ""
	srcAddressBytes := byteArray[48:80]
	srcAddressString := ""

	addressString = ByteArrayToString(addressBytes, "!")
	srcAddressString = ByteArrayToString(srcAddressBytes, "!")


	newLink.Addr = addressString
	newLink.SrcAddr = srcAddressString
	newLink.Latency, _ = strconv.ParseFloat(latencyString, 64)
	return newLink
}

func ByteArrayToString(input []byte, endChar string) string {
	outputString := ""

	for _, value := range input {
		if string(value) != endChar {
...
func Bounce(buf []byte) {
	...
	file, err := os.OpenFile(bouncesFilename, os.O_APPEND|os.O_WRONLY, 0600)
	CheckError(err)
...
func GetData() {
	...
	// CREATE FILE
	bouncesFilename = strconv.Itoa(localAddr.Port) + ".txt"
	file, err := os.Create(bouncesFilename)
	CheckError(err)
	defer file.Close()
...
*/
func StartOracle(oracleAddrString string, databaseFilename string) {
	...
	//Setup the data file
	bouncesFilename = strconv.Itoa(oracleAddr.Port) + ".txt"
	file, err := os.Create(bouncesFilename)
	CheckError(err)
	defer file.Close()
...
/* UpdateDatabase() takes the databaseFilename and updates the links in that
   csv file
*/
func UpdateDatabase(inBuf []byte) {

	// Open file and check for errors
	file, err := OpenCSVFile(databaseFilename)
	if err != nil {
		fmt.Println("ERROR: File not opened properly")
		return
	}

	// Extract packet data and pull link from it as a new link object
	packetData := ExtractPacketData(inBuf)
	newLink := ByteArrayToLink(packetData)

	// Update that link in the routing table
	file.UpdateRowWithLink(&newLink)
}
...
type Row []string
type Matrix []Row // A 2D matrix used to store CSV data
...
/* Opens a CSV file and stores it into a CSV file object, which
   contains a filename and it's data as a matrix
*/
func OpenCSVFile(filename string) (CSVfile, error) {
  var newFile CSVfile
  var err error
  newFile.filename = filename
  newFile.data, err = ToArray(filename)

  return newFile, err
}


/* Converts a CSV file into a matrix (an array of arrays, where each
   sub array is a row of the matrix)
*/
func ToArray(fileName string) (Matrix, error) {

  var output Matrix // output array
  var err error // output error
  ...
  for scanner.Scan() {
    ...
    cell := ""
    // loop through ever character in the line
    var lineArray []string
    for i, value := range line {
      char := string(value)
      if (char != ",") && (i <= len(line)-1) {
        // if char is not a comma, append it to the cell's value
        cell += char
      } else {
        // if char is a comma, push to array and reset cell
        lineArray = append(lineArray, cell)
        cell = ""
      }
    }

    // Flush the cell before moving to new line
    lineArray = append(lineArray, cell)
    output = append(output, lineArray)
...
/* This function takes inputted matrix data and writes it to a csv file, with
   the name given as an input. ".csv" must be marked in the filename
*/
func (inputData Matrix) WriteToCSV(filename string) {

  // Create string to be written, comma dilimited
  writeString := ""
  for _, row := range inputData {
    for i, columnValue := range row {
      writeString += columnValue
      if i < len(row)-1 {
        writeString += ","
      }
    }
...
/* UpdateRowWithLink() takes a given input file and
*/
func (inputFile CSVfile) UpdateRowWithLink(link *Link) {

  data := inputFile.data // file's data as matrix
  latency := link.Latency // link's latency value
	// Remember: the node who's link this came from is the destination
	// address of the probe!
  dstAddr := link.SrcAddr // link's destination address
  srcAddr := link.Addr // link's source address

  // Get the row for the corresponding link
  row, n, err := data.GetMatchingRow(srcAddr, dstAddr)
  if err != nil {
    fmt.Println("ERROR: Row not found, file not updated")
    return
  }

  // Change the latency value to match the link and update the matrix
  row[2] = strconv.FormatFloat(latency, 'f', 15, 64)
  data[n] = row

  // Write the new matrix to the file
  data.WriteToCSV(inputFile.filename)
...
// Function that returns the row containing a given source and destination addr
func (inputArray Matrix) GetMatchingRow(source string, dest string) (Row, int, error) {
  rowNum := -1
  var foundRow Row
  // loop through input to find the matching source Row
  for i, value := range inputArray {
    if value[0] == source {
      // if source matches, check destination
      if value[1] == dest {
        rowNum = i
      }
    }
  ...
  // Check for row not found
  if rowNum == -1 {
    err = errors.New("ROW NOT FOUND")
  } else {
    foundRow = inputArray[rowNum]
    err = nil
  }

  // Return found row
  return foundRow, rowNum, err
}
