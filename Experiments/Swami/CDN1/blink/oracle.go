...
func handlePacketTrackUpdate(buffer []byte) {
	...
	if !packet.Time1.IsZero() {
		p := Packet{Number: packet.Number, Time1: packet.Time1, Time2: PacketTrackTable[SID][packet.Number].Time2, Time3: PacketTrackTable[SID][packet.Number].Time3}
		PacketTrackTable[SID][packet.Number] = p
	}

	if !packet.Time2.IsZero() {
		p := Packet{Number: packet.Number, Time1: PacketTrackTable[SID][packet.Number].Time1, Time2: packet.Time2, Time3: PacketTrackTable[SID][packet.Number].Time3}
		PacketTrackTable[SID][packet.Number] = p
	}

	if !packet.Time3.IsZero() {
		p := Packet{Number: packet.Number, Time1: PacketTrackTable[SID][packet.Number].Time1, Time2: PacketTrackTable[SID][packet.Number].Time2, Time3: packet.Time3}
		PacketTrackTable[SID][packet.Number] = p
	}
