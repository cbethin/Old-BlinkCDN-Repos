...
func Bounce(buf []byte) {
	...
	packetAsPacket := ByteArrayToJSONToPacket(packetDataAsBuffer) // <-- ERROR HAPPENS HERE -- filename makes it too long
	...
	if _, ok := DecisionTable[SID]; ok {
		...
		if destAddr.String() != LocalAddrString {
			...
			for i, value := range DecisionTable[SID] {
				if value == LocalAddrString {
					...
					if i == len(DecisionTable[SID])-1 {
						if destAddr.String() == ":0" {
							fmt.Println("Receiving", packetNumber)
							trackPacketDelay(packetAsPacket.Number, hopNumber, receivedAtTime, SID)
							ListenVideo(packetAsPacket.Data, packetAsPacket.Number)
							return
						} else {
							if string(packetAsPacket.Data) == "TRANSFER COMPLETE" {
								outputBuf := MakeBlinkPacket(SESSION_ID_TEST, localAddr, oracleAddr, BlinkSignal, []byte("success"))
								_, err := conn.WriteToUDP(outputBuf, oracleAddr)
								CheckError(err)
								fmt.Println("Sent blink signal")
							}
			...
			if nextNodeAddrString == "" {
				fmt.Println("DECISION TABLE:", DecisionTable[SID])
				fmt.Println("Next node not found")
				return
			} else {
				fmt.Println("Forwarding to:", nextNodeAddrString)
			}
			...
			trackPacketDelay(packetNumber, hopNumber, receivedAtTime, SID)
...
func trackPacketDelay(packetNumber int, hopNumber int, t time.Time, SID string) {
