...
func handlePacketTrackUpdate(buffer []byte) {
	...
	if !PacketTrackTable[SID][packet.Number].Time1.IsZero() && !PacketTrackTable[SID][packet.Number].Time2.IsZero() {
		...
		refineData(PacketTrackTable[SID][packet.Number])
	}
}
...
func refineData(p Packet) { // SET BACK TO MAP
	/*
		Takes t1 and t2 and finds t total then sends that,
		packet number and service time to function firebaseData
	*/
