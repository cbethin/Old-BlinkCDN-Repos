...
function placeBid(roomName, bid, socket) {
  ...
  if (!rooms[roomName].services.bid.isOnBreak) {
...
function setupBidUser(userID, roomName) {
  ...
  if (sockets.hasOwnProperty(userID)) {
    ...
    if (rooms[roomName]) {
      ...
      socket.emit('bid setup', topSixBids, rooms[roomName].services.bid.bidCount);
    }
  }
...
function startAuctionTimer(roomName) {
  ...
  rooms[roomName].services.bid.isOnBreak = false;
...
function startBreakTimer(roomName) {
  ...
  rooms[roomName].services.bid.isOnBreak = true;
}
