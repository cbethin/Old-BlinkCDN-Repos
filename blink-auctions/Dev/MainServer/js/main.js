...
var button;
var nextBidStr;
var bidArray = []; // Holds list of bids
var user = {}; // Object containing this user's info
var services = { // Object containing objects housing the bid and stream APIs
  "bid": bidEng,
  "stream": streamEng
}
...
function setupSocket() {
  ...
  window.beforeunload = function() {
    socket.emit('userDisconnect', user.userID, roomName)
...
function updateGameboard() {
  ...
  for (var i=0; i < numPlayersOnScoreboard; i++) {
    ...
    if(i != 0) {
      var opacity = 0.7-((0.7-0.05)/6)*i
      $(gameboardLabels[i]).css('opacity', opacity);
